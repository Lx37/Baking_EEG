...
master_log_file = os.path.join(
    ...
    datetime.now().strftime(f'master_submitit_ALL_GROUPS_INTRA_%Y-%m-%d_%H-%M-%S.log')
)
...
def main():
    ...
    logger.info(f"Groupes ciblés: {GROUPS_TO_PROCESS}")

    # Configuration du nombre de CPUs pour Slurm
    # Vous pouvez le rendre configurable ou l'importer de N_JOBS_PROCESSING si souhaité
    ...
    log_folder_submitit_jobs = os.path.abspath(
        os.path.join(log_dir_submitit_master,
                     f"slurm_jobs_ALL_GROUPS_INTRA_{current_time_str}")
    )
        ...
        with executor.batch():
            for group_name_to_analyze in GROUPS_TO_PROCESS:
                ...
                func_kwargs_group_analysis = {
                    ...
                    "enable_verbose_logging": True,  # Mettre à False pour moins de logs
                    "n_jobs_for_each_subject": n_jobs_for_ops_in_job,
                    "n_jobs_for_group_cluster_stats": n_jobs_for_ops_in_job,

                    # Paramètres de pipeline/modèle (utilisation des constantes importées)
                    ...
                    # Paramètres de statistiques (utilisation des constantes importées)
                    ...
                    # Vous le voulez probablement pour l'analyse de groupe
                    ...
                    # Paramètres de chargement et d'exécution (utilisation des constantes importées)
                    "save_results_flag": SAVE_ANALYSIS_RESULTS,
                    "generate_plots_flag": GENERATE_PLOTS,
                    # Pour les jobs intra-sujets
    ...
    for item in group_jobs_info:
        ...
        try:
            # La fonction execute_group_intra_subject_decoding_analysis retourne un dictionnaire
            # {subject_id: global_auc_score, ...}
