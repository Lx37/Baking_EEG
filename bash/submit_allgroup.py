...
# -*- coding: utf-8 -*-
"""
Script de soumission Submitit pour des analyses de groupe (intra-sujet).
Ce script configure et soumet plusieurs jobs Slurm, un pour chaque groupe de sujets,
en utilisant une structure robuste pour la gestion des chemins, le logging et la soumission.
"""
...
# --- Détermination de PROJECT_ROOT_FOR_PYTHONPATH (copié du modèle) ---
# NOTE: Cette section est volontairement verbeuse pour le débogage.
# Ligne CORRECTE
project_root_tentative = os.path.abspath(
    os.path.join(CURRENT_SCRIPT_DIR, ".."))

if os.path.isdir(project_root_tentative):
    PROJECT_ROOT_FOR_PYTHONPATH = project_root_tentative
    print(
        f"INFO: PROJECT_ROOT_FOR_PYTHONPATH confirmé : {PROJECT_ROOT_FOR_PYTHONPATH}")
else:
    print(
        f"AVERTISSEME_NT: Le chemin '{project_root_tentative}' n'existe pas. Recherche de solution de repli.")
    potential_paths = [
    ...
    for path_candidate in potential_paths:
        # On vérifie la présence d'un dossier connu, comme 'examples' ou 'config'
        if os.path.isdir(os.path.join(path_candidate, "examples")):
            PROJECT_ROOT_FOR_PYTHONPATH = path_candidate
            print(
                f"INFO: PROJECT_ROOT_FOR_PYTHONPATH trouvé via fallback : {PROJECT_ROOT_FOR_PYTHONPATH}")
            found_path = True
            break
    if not found_path:
...
MASTER_LOG_FILE_NAME = datetime.now().strftime(
    'master_submitit_ALL_GROUPS_INTRA_%Y-%m-%d_%H-%M-%S.log'
)
...
logger.info(
    "--- Script de soumission Submitit pour analyses de groupe démarré ---")
logger.info(
...
try:
    ...
    from config.decoding_config import (
        ...
        N_PERMUTATIONS_INTRA_SUBJECT, COMPUTE_INTRA_SUBJECT_STATISTICS, N_PERMUTATIONS_GROUP_LEVEL,
        GROUP_LEVEL_STAT_THRESHOLD_TYPE, T_THRESHOLD_FOR_GROUP_STAT_CLUSTERING,
        INTRA_FOLD_CLUSTER_THRESHOLD_CONFIG, CONFIG_LOAD_SINGLE_PROTOCOL,
        SAVE_ANALYSIS_RESULTS, GENERATE_PLOTS,
    )
    logger.info("Importations depuis 'config' et 'utils' réussies.")
except (ModuleNotFoundError, ImportError) as e:
    logger.critical(
        f"ERREUR CRITIQUE lors de l'importation de modules projet : {e}", exc_info=True)
    logger.critical(
        f"Vérifiez PROJECT_ROOT_FOR_PYTHONPATH ('{PROJECT_ROOT_FOR_PYTHONPATH}') et sys.path.")
    sys.exit(1)
...
logger.info(
    f"Racine du projet à utiliser pour PYTHONPATH dans le job Slurm: {PROJECT_ROOT_ON_CLUSTER_FOR_JOB}")
...
def main_submission_logic():
    ...
    try:
        ...
        user_for_paths = os.environ.get(
            'LOGNAME') or os.environ.get('USER', "unknown_user_env")
    logger.info(
        f"Utilisateur détecté pour la configuration des chemins : {user_for_paths}")

    base_input_path, base_output_path = configure_project_paths(user_for_paths)
    if not os.path.isdir(base_input_path):
        logger.critical(
            f"ERREUR: Le chemin des données d'entrée '{base_input_path}' n'existe pas. Arrêt.")
        sys.exit(1)
    ...
    GROUPS_TO_PROCESS = ['controls', 'del', 'nodel']
    logger.info(f"Groupes ciblés pour l'analyse : {GROUPS_TO_PROCESS}")

    # Configuration des ressources Slurm
    SLURM_CPUS_PER_GROUP_JOB = 40
    SLURM_MEMORY_PER_JOB = "80G"
    SLURM_TIMEOUT_MINUTES = 1000 * 60
    SLURM_PARTITION = "CPU"
    ...
    submitit_job_log_folder = os.path.abspath(os.path.join(
        LOG_DIR_SUBMITIT_MASTER, f"slurm_jobs_ALL_GROUPS_INTRA_{current_time_str}"
    ))
    os.makedirs(submitit_job_log_folder, exist_ok=True)
    logger.info(
        f"Logs Submitit spécifiques aux jobs Slurm dans : {submitit_job_log_folder}")
        ...
        with executor.batch():
            for group_name_to_analyze in GROUPS_TO_PROCESS:
                if group_name_to_analyze not in ALL_SUBJECT_GROUPS:
                    logger.warning(
                        f"Groupe '{group_name_to_analyze}' non trouvé dans la configuration. Ignoré.")
                    continue

                subjects_for_this_group = ALL_SUBJECT_GROUPS[group_name_to_analyze]
                if not subjects_for_this_group:
                    logger.warning(
                        f"Aucun sujet défini pour le groupe '{group_name_to_analyze}'. Ignoré.")
                    continue

                logger.info(
                    f"Préparation du job pour le groupe '{group_name_to_analyze}' ({len(subjects_for_this_group)} sujets)")
                ...
                func_kwargs_group_analysis = {
                    ...
                    "compute_group_level_stats_flag": True,  # Essentiel pour une analyse de groupe
                ...
                job = executor.submit(
                    execute_group_decoding_wrapper, **func_kwargs_group_analysis)
                group_jobs_info.append(
                    {"job_object": job, "group_name": group_name_to_analyze})
                logger.info(
                    f"    -> Job pour le groupe '{group_name_to_analyze}' soumis au batch.")

    except Exception as e:
        logger.critical(
            f"Erreur critique lors de la configuration ou de la soumission du batch : {e}", exc_info=True)
        return
    ...
    logger.info(
        f"{len(group_jobs_info)} job(s) de groupe soumis avec succès. Attente des résultats...")
    ...
    for item in group_jobs_info:
        ...
        logger.info(
            f"Attente du résultat pour le groupe : {group_name} (Job ID: {job_id_str})...")
        ...
        try:
            ...
            logger.info(
                f"Job {job_id_str} (Groupe: {group_name}) terminé. État final : {job_final_state}")
            ...
            if isinstance(output_subject_auc_dict, dict) and output_subject_auc_dict:
                ...
                logger.info(
                    f"  -> Résultat pour {group_name} : Moyenne AUCs = {mean_auc_for_group:.3f} (N={num_subjects_processed} sujets).")
            else:
                logger.warning(
                    f"  -> Résultat inattendu/vide pour {group_name}. Type reçu : {type(output_subject_auc_dict)}")
            ...
            logger.error(
                f"Le job pour le groupe {group_name} (ID: {job_id_str}) A ÉCHOUÉ.", exc_info=False)
            logger.error(f"  Message de Submitit : {e_failed}")
            logger.error(
                f"  Veuillez consulter les logs du worker dans : {submitit_job_log_folder}")
            group_analysis_results_summary[group_name] = {
                'mean_auc': np.nan, 'num_subjects': 0, 'job_id': job_id_str, 'state': 'FAILED'}
        except Exception as e_result:
            logger.error(
                f"Erreur lors de la récupération du résultat pour le groupe {group_name} (ID: {job_id_str}) : {e_result}", exc_info=True)
            group_analysis_results_summary[group_name] = {
                'mean_auc': np.nan, 'num_subjects': 0, 'job_id': job_id_str, 'state': f'ERROR_RETRIEVAL (état submitit: {getattr(job_obj, "state", "N/A")})'}

    logger.info("\n" + "="*25 +
                " RÉSUMÉ FINAL DES ANALYSES DE GROUPE " + "="*25)
    all_jobs_ok = True
    for group, summary in group_analysis_results_summary.items():
        auc_str = f"{summary['mean_auc']:.3f}" if pd.notna(
            summary['mean_auc']) else 'N/A'
        logger.info(
            f"Groupe: {group:<10} | Job ID: {summary['job_id']:<15} | État: {summary['state']:<10} | Moyenne AUC: {auc_str:<5} | N Sujets: {summary['num_subjects']}")
        if summary['state'] != 'DONE':
            all_jobs_ok = False

    if all_jobs_ok and group_analysis_results_summary:
        logger.info(
            "Toutes les analyses de groupe semblent s'être terminées avec succès.")
    else:
        logger.warning(
            "Au moins un job de groupe a échoué ou a rencontré une erreur. Veuillez vérifier les logs.")
    logger.info("="*81)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.critical(
            f"Erreur non gérée dans main_submission_logic : {e_main}", exc_info=True)
        sys.exit(1)
    finally:
        logger.info(
            f"--- Fin du script de soumission principal. Log complet dans : {MASTER_LOG_FILE_PATH} ---")
