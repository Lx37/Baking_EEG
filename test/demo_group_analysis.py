    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                logger.info(
                    "✅ Fonction appelée sans erreur (données manquantes attendues)")
                logger.info(f"📊 Résultat type: {type(result)}")

            except FileNotFoundError:
                logger.info(
                    "✅ FileNotFoundError attendue (données de test manquantes)")
...
def main():
    ...
    if passed_tests == total_tests:
        ...
        logger.info(
            "Le script run_decoding_one_group_pp.py est prêt à l'emploi.")
    else:
