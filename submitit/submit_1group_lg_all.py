...
def main():
    ...
    jobs = []
    logger.info(f"Préparation de la soumission d'un job LG par participant pour le groupe {group}...")

    for subject in subjects:
        job_folder = os.path.join(log_dir, f"slurm_job_LG_{group}_{subject}_{timestamp}")
        executor = submitit.AutoExecutor(folder=job_folder)
        executor.update_parameters(
            name=f"decoding_LG_{group}_{subject}",
            timeout_min=120000,
            slurm_partition="CPU",
            slurm_mem="79G",
        ...
        logger.info(f"Job {job.job_id} soumis pour le sujet {subject} du groupe LG {group}.")
        jobs.append((subject, job))

    logger.info(f"--- Tous les jobs LG pour le groupe {group} soumis. En attente des résultats... ---")

    for subject, job in jobs:
        try:
            result = job.result()
            logger.info(f"Job {job.job_id} (Sujet: {subject}, Groupe LG: {group}) a terminé avec le statut {job.state}.")
        except FailedJobError as e:
            logger.error(f"Job {job.job_id} (Sujet: {subject}, Groupe LG: {group}) A ÉCHOUÉ: {e}")
