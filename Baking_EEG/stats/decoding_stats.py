...
import os
import numpy as np
import scipy.stats # For t-tests and Wilcoxon
from mne.stats import (
    permutation_cluster_1samp_test, # For cluster-based permutation tests
    ttest_1samp_no_p, # MNE's version of 1-sample t-test (no p-value, for permutations)
    fdr_correction, # For False Discovery Rate correction
)

logger_stats_decoding = logging.getLogger(__name__)
def _custom_stat_function_ttest_1samp(
    data_array,
    sigma_val=0.0, # Corresponds to popmean in scipy.stats.ttest_1samp
    method_mne="relative" # MNE's t-test method
):
    """
    ...
    Args:
        data_array (np.ndarray): Input data array of shape (n_observations, n_features...)
        ...
        method_mne (str): Method for MNE's t-test calculation (default: 'relative').
                          'relative' uses a pooled variance estimate if data_array is 2D+
                          and represents different conditions. For 1-sample, it's standard.

    Returns:
        np.ndarray: Calculated t-values, with NaNs replaced by 0 for robust clustering.
    """
...
# === MAIN STATISTICAL TEST FUNCTIONS ===
...
def perform_cluster_permutation_test(
    ...
    if effective_stat_function is not None:
        ...
        logger_stats_decoding.info(f"  Using custom stat_fun: {effective_stat_function.__name__}")
    else: # MNE default ttest_1samp_no_p will be used if 'stat_fun' not in test_args_mne
        logger_stats_decoding.info("  Using MNE's default stat_fun (ttest_1samp_no_p).")
  ...
  # === POST-PROCESSING RESULTS ===
    ...
    cluster_definitions_masks_for_output = [m for m in processed_cluster_masks if m is not None]
    ...
    if cluster_definitions_masks_for_output and cluster_p_values is not None: 
        ...
        num_clusters_to_log = min(len(cluster_definitions_masks_for_output), len(cluster_p_values))
...
def perform_pointwise_fdr_correction_on_scores(
    ...
    """
    Performs a pointwise one-sample t-test for each feature against a chance level,
    followed by FDR correction on the resulting p-values.

    This function is useful for identifying time points or spatial locations where
    decoding scores are significantly different from chance, while controlling
    the false discovery rate.

    Args:
        input_data_array (np.ndarray): Input data with observations as the 1st dimension.
                                      Shape: (n_observations, n_features) or (n_obs,) for a single feature.
        chance_level (float): Value to test against (chance level).
        alpha_significance_level (float): Alpha level for FDR (default: 0.05).
        fdr_correction_method (str): FDR method - 'indep' (Benjamini-Hochberg) or
                                    'negcorr' (Benjamini-Yekutieli for negative correlations).
        alternative_hypothesis (str): 'two-sided', 'greater', or 'less' for scipy.stats.ttest_1samp.

    Returns:
        tuple: (observed_t_values, fdr_significant_mask, fdr_corrected_p_values)
            - observed_t_values (np.ndarray): T-values, shaped like the original features.
            - fdr_significant_mask (np.ndarray): Boolean mask of significance after FDR.
    ...
    if original_ndim_fdr > 2:
        ...
        data_for_ttest_fdr = input_data_array.reshape(n_obs_fdr, n_total_features_fdr)
        logger_stats_decoding.debug(
    ...
    if data_for_ttest_fdr.shape[0] < 2:
        ...
        original_feature_shape = input_data_array.shape[1:] if original_ndim_fdr > 1 else (1,)
        return (
    ...
    fdr_significant_mask_flat = np.full_like(uncorrected_p_values_flat, False, dtype=bool)
    fdr_corrected_p_values_flat = np.full_like(uncorrected_p_values_flat, np.nan, dtype=float)
    ...
    original_feature_shape_out = input_data_array.shape[1:] if original_ndim_fdr > 1 else (1,)
    ...
    if original_ndim_fdr == 1 and observed_t_values_flat.size == 1:
        ...
        observed_t_values_out = observed_t_values_flat.reshape(original_feature_shape_out)
        fdr_significant_mask_out = fdr_significant_mask_flat.reshape(original_feature_shape_out)
        fdr_corrected_p_values_out = fdr_corrected_p_values_flat.reshape(original_feature_shape_out)
...
def compare_global_scores_to_chance(
    ...
    scores_cleaned_global = np.array(global_scores_array).flatten()  # Ensure 1D array
    scores_valid_for_test_global = scores_cleaned_global[~np.isnan(scores_cleaned_global)]  # Remove NaNs
    ...
    # === NON-PARAMETRIC WILCOXON SIGNED-RANK TEST ===
        ...
        raise ValueError(f"Unsupported statistical test type: {statistical_test_type}")
...
__all__ = [
    ...
    "_custom_stat_function_ttest_1samp",
]