...
def read_raw(patient_info):
    ...
    if patient_info['EEG_system'] == 'EGI':
        if patient_info['ID_patient'] == 'TT02':
            data = mne.io.read_raw_egi(
                patient_info['data_fname'], eog=None, misc=None, include=None, preload=True, verbose=None)
        else:
            data = mne.io.read_raw_egi(
                patient_info['data_fname'], eog=None, misc=None, exclude=None, include=None, preload=True, verbose=None)
...
def set_montage(data, patient_info, cfg, verbose, plot):
    ...
    if patient_info['EEG_system'] == 'EGI':
        ...
        coordinatesfile = patient_info['raw_data_dir'] + \
            '/montage modifie_gNautilusPro_JB.xyz'
        # , head_size=0.095, coord_frame=None, *, verbose=None)
        montage = mne.channels.read_custom_montage(coordinatesfile)
        print('montage loaded ch_names : ', montage.ch_names)
        data.set_channel_types(
            {'EEG 000': 'misc', 'EEG 033': 'misc', 'EEG 034': 'misc'})
...
def create_oculars(data, patient_info, cfg, verbose, plot):  # function just for ocular mov
    ...
    # vertical mov of each eye + horizontal of both
...
def preprocess(patient_info, cfg, save=False, verbose=True, plot=True):
    ...
    if patient_info['protocol'] != 'Resting':
        ...
        # if not resting stqte, then functions prints conditions of the protocol
        print(event_id)
    # set montage
    ...
    if plot:
        # scaling set to eeg=20e-6, eog=150e-6
        data.plot(show_options=True, block=True, title='Raw EEG data')
    ...
    data.notch_filter(np.arange(50, 200, 50), picks=picks_eeg,
                      filter_length='auto', phase='zero-double')
    # band pass filter around 1-25Hz
    data.filter(cfg.highpass, cfg.highcut, method='fir',
                phase='zero-double', fir_design='firwin2')

    if plot:
        picks_psd_plot = mne.pick_types(data.info, eeg=True)
        fig, ax = plt.subplots(1)
        data.plot_psd(ax=ax, area_mode='range', tmax=10.0,
                      picks=picks_psd_plot, average=False, show=False)
        ax.set_title('FFT after filtering for patient ' +
    ...
    if patient_info['EEG_system'] == 'EGI' and patient_info['protocol'] != 'Resting':
        events = mne.find_events(
            datacheck, stim_channel='STI 014')  # stimulus channel
        evoked = mne.Epochs(datacheck, events=events,
                            picks='eeg', tmin=-0.2, tmax=1.2).average()
        evoked.plot(
            titles='Global average triggers, any new bad sensors? (the actual bads are not showned) Note them and indicate in next EEG plot')
    ...
    print('Bad channels : %s for patient %s',
          data.info['bads'], patient_info['ID_patient'])

    #################### Interpollation ####################

    # used only to calculate average ref (for now)
    print('###### Interpolate bad chans ######')
    ...
    if plot:
        data.plot_sensors(kind='3d', ch_type='eeg',
                          title='Sensory positions, Red ones are indicated as bads ')

    # ########### Create HEOG and VEOG chans ###########

    print('###### Create oculars ######')
    if patient_info['EEG_system'] == 'EGI':
        raw_VEOGL, raw_VEOGR, raw_HEOG = create_oculars(
            data, patient_info, cfg, verbose, plot)
        # TODO Only VEOG added for now
        data.add_channels([raw_VEOGL, raw_VEOGR], force_update_info=True)
    ...
    if patient_info['protocol'] != 'Resting':
        ...
        for x in range(events.shape[0]):  # loop over rows
            ...
            good_events = events[(event_names != b'Rest') & (event_names != b'Code') & (
                event_names != b'star') & (event_names != b'rest'), :]  # all events where names is not 'rest'
    ...
    if save:
        ...
        data_name = data_name + \
            ...
            patient_info['protocol'] + cfg.prefix_processed
        ...
        if patient_info['protocol'] != 'Resting':
            ...
            if patient_info['EEG_system'] == 'EGI':
                ...
                nameStimDict = patient_info['data_save_dir'] + \
                    cfg.stimDict_path
...
# def preprocess_mircomed(cfg, data_fname, sub_name, bad_sub_chan=[], save=False, verbose=True, plot=True):
    ...
    if plot:
        # scaling set to eeg=20e-6, eog=150e-6
        data.plot(show_options=True, block=True)

    #################### Filters ####################
    # notch filter around 50hz
    picks_eeg = mne.pick_types(data.info, eeg=True)
    data.notch_filter(np.arange(50, 100, 50), picks=picks_eeg,
                      filter_length='auto', phase='zero-double')
    # band pass filter around 1-25Hz
    data.filter(cfg.highpass, cfg.highcut, method='fir',
                phase='zero-double', fir_design='firwin2')
    ...
    if plot:
        ...
        data.plot_psd(area_mode='range', tmax=10.0,
                      picks=picks_psd_plot, average=False)
    ...
    data.plot(show_options=True,
              title='Indicate bad sensors manually', block=True)
    print(data.info)

    if plot:
        data.plot_sensors(kind='3d', ch_type='eeg',
                          title='Sensory positions, Red ones & Cz are indicated as bads ' + sub_name)

    logger.info('Bad channels : ,%s', data.info['bads'])

    #################### Interpollation ####################
    # used only to calculate average ref (for now)
    print('Bad channels are interpolated')
    data.interpolate_bads(reset_bads=False)
    # Pay ATTENTION! All bad channels are interpolated, this we did to be able to interpolate the Cz channel with only good ones, and is necessary for source reconstruction.
    # PAY ATTENTION : keep using (for pick_types, epochs etc.) " exclude=[] " otherwise the bads get deleted!
    if plot:
    ...
    if save:
        ...
        data_name = cfg.data_preproc_path + \
            data.info['subject_info']['his_id'] + prefix_processed
        print("Saving data : " + data_name)
        data.save(data_name, overwrite=True)
        logger.info("Saved preprocessed data " + data_name)
    ...
    if plot:
        scalings = dict(eeg=20e-6, eog=100e-6, misc=20e-6)
        data.plot(show_options=True, scalings=dict(
            eeg=20e-6, eog=100e-6), block=True)
